...
namespace Microsoft.BotBuilderSamples
{
    ...
    public class BasicBot : IBot
    {
        ...
        public const string Shinhani = "신한아이";
...

        {
            ...
            // lParam 부분을 ref COPYDATASTRUCT로 바꾸어 overloading하는 방법도 있지만 아래 Marshaling을 쓰면 굳이 그럴 필요 없다.
        ...
        { // Win32 API의 그것과 비슷하게 선언

            public IntPtr dwData;

            public int cbData;

            public IntPtr lpData; // IntPtr가 LPVOID 역할을 한다.
                                ...
                                case BuyIntent:
                                    {
                                        if(bFundEntities == true)
                                        {
                                            ...
                                            actions.Add(new CardAction() { Title = "신한아이연결", Value = "신한아이", Type = ActionTypes.ImBack });
                                            response.Attachments.Add(
                                                new HeroCard
                                                {
                                                    Title = "펀드 매수 화면번호는 5861번입니다.",
                                                    Buttons = actions,
                                                }.ToAttachment());
                                            await dc.Context.SendActivityAsync(response);
                                        }
        ...
        {
            IntPtr hwndRcvr = Win32.FindWindow(null, "ReceiverWindow"); // ReceiverWindow라는 window class가 등록돼 있고, 현재 실행 중이다.
            ...
            cds.cbData = Marshal.SizeOf(data);    // 그냥 sizeof를 쓰면 안 되고, Marshal.SizeOf를 써야 한다.
